<aside id="aside" class="fixed inset-y-0 left-0 flex-wrap items-center justify-between block p-0 my-4 transition-all duration-200 bg-white border-0 shadow-none xl:ml-4 dark:bg-gray-950 ease-soft-in-out z-20 rounded-2xl xl:translate-x-0 xl:bg-transparent ps ps--active-y -translate-x-full shadow-soft-xl max-w-56 overflow-hidden ">
    ...
    <div class="items-center block w-auto max-h-screen  h-screen overflow-auto h-sidenav grow basis-full">
        ...
        @if(auth()->user()->isAdmin())
            ...
            <li class="mt-0.5 w-full">
                <a class="py-2.7 text-sm ease-nav-brand my-0 mx-4 flex items-center whitespace-nowrap
                {{ (Route::currentRouteName()== "auditlogs.index") ? 'shadow-soft-xl rounded-lg bg-white' : '' }} px-4 font-semibold text-slate-700 transition-colors" href="{{route("classrooms.index")}}">
                <div class="{{ Route::currentRouteName() == "auditlogs.index" ? 'bg-gradient-to-tl from-purple-700 to-pink-500' : '' }} shadow-soft-2xl mr-2 flex h-8 w-8 items-center justify-center rounded-lg bg-white bg-center stroke-0 text-center xl:p-2.5">
                    <i class="fa-solid fa-clipboard-check  {{Route::currentRouteName() == "auditlogs.index" ? 'text-white' : ''}}"></i>
                </div>
                <div class="sidebar-icon">
                    <span class="ml-1 duration-300 opacity-100 pointer-events-none ease-soft">{{trans("main-sidebar.auditlogs.index")}}</span>
                </div>
                </a>
            </li>
